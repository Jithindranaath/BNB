...
- `services/orchestrator/Dockerfile` (context = repo root, editable install so reference JSON / manifests resolve; **`anvil` 1.8.1 copied in from `ghcr.io/foundry-rs/foundry:v1.8.1`** — bsc-sentry + pcs-yield's gate fork BSC per hire) + `infra/deploy/entrypoint.sh` (`serve` = `alembic upgrade head` then uvicorn on `$PORT`; `migrate` = migrations only) + `.dockerignore`. Image builds (~1 GB); ran against the local Postgres → `/healthz` `status: ok` (db + rpc ok, redis down tolerated), `/agents` real; `anvil --version` inside the image confirms 1.8.1.
